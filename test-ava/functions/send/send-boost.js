...
async function sendBoost(t, node1, node2, replyMessage, amount, tribe){
//NODE1 SENDS TEXT MESSAGE TO NODE2

  //get balances of both nodes before boost
  const [boosterBalBefore, boosteeBalBefore] = await boostBalances(t, node1, node2)
  ...
  t.truthy(tribeId, "tribeId should exist")
    ...
    t.true(Math.abs(((boosteeBalAfter - boosteeBalBefore) - amount)) <= r.allowedFee, "boostee should have gained amount")
