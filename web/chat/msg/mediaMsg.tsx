...
export default function MediaMsg(props) {
  ...
  const { message_content, chat, media_token, media_type, boosts_total_sats } =
    props;
  ...
  const { data, loading, trigger, paidMessageText, filename } =
    useCachedEncryptedFile(props, ldat);
    ...
    <Wrap>
      ...
      {isPaidMsg && (
        <>
          ...
          {paidMessageText && (
            <Content style={{ paddingTop: 40 }}>{paidMessageText}</Content>
          )}
        </>
      )}
      ...
      <VidWrap>
        <Player>
          <source
            src={data}
            type={
              type === "video/mov" || type === "video/quicktime"
                ? "video/mp4"
                : type
            }
          />
